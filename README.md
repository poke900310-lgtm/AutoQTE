...
**a mod manager:** install the archive and choose the **UE4SS (Lua mods)** install
type. a mod manager strips the `Data` folder and deploys into
...
> changes; `Engine\Binaries` stays `Win64`. a mod manager resolves this itself.
...
settings with it. a mod manager does the right thing automatically. Close the game
...
Remove the mod in a mod manager, or delete
