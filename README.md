...
| Drink Blood | Hold duration decides unconscious vs. drained, which sets a persistent fact tag and is read by quest conditions — the hold *is* the choice. Automation was built and removed in 1.0.6: `bButtonPressed` is never read, driving `TickDrinking` advances the stages but the game sees no input (no drain meter), and `InputDrinkBlood` is edge-triggered, so a second press aborts the feed. Your call, not the mod's. |
...
| Key | Claimed by |
|---|---|
| F6 | another mod (open/close), another mod |
| F7 | another mod (reload), another mod |
| F8 | another mod (toggle) |
| F9 | another mod (scan), another mod |
| F10 | UE4SS console, another mod |
| F11 | another mod, another mod |
| F12 | another mod, and Steam's screenshot key |
...
F1, F2 and F3 are claimed by another mod on this game, so they make poor
...
- **Ships one mod folder.** No shared file is in the archive — no
  `UE4SS-settings.ini`, no `mods.txt`, no `mods.json`, no pak, no `modloader`
...
- **Depends on no shared Lua library** — not `UEHelpers`, not `modloader` —
...
- **Writes to exactly one shared thing**, and only during a skip: `RenderOpacity`
  ...
  knowing: a mod that independently sets the same widget to `0.0` — another mod'
...
- **Known HUD interaction.** *another mod* lists `WBP_DIS_Prompt_New_C` in
  ...
  shouldn't be, set `HidePrompt = false` in `AutoQTE.ini`, or drop that widget from another mod'
  watch list. *another mod* uses the same idiom but does not target the DIS
  prompt.
- **Input remaps are irrelevant.** AutoQTE never simulates a keypress — it calls
  ...
  Enhanced Input remap (*another mod & Remap*) do not affect it.
...
**`AutoQTE v1.0.6 loaded (0 blocklist patterns)` but nothing happens in a scene**
